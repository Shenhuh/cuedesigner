...
class JointController extends Controller
{
    ...
    public function update(Request $request, $id)
    {
        try {
            ...
            if ($request->hasFile('image')) {
                ...
                if ($joint->imagePath) {
                    Storage::disk('public')->delete($joint->imagePath);
                }

                // Store the new image and update the image path in validated data
                $validatedData['imagePath'] = $request->file('image')->store('joints', 'public');
            } else {
                // Keep the existing image path if no new image is uploaded
                $validatedData['imagePath'] = $joint->imagePath;
            }

            // Update the texture in the database
            $joint->update($validatedData);
    ...
    public function destroy($id)
    {
        try {
            ...
            $joint->delete(); // This will set the 'deleted_at' column
            
            return response()->json(['message' => 'Joint deleted successfully!'], 200);
        } catch (\Exception $e) {
