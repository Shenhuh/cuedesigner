...
class WoodController extends Controller
{
    public function store(Request $request)
    {
        try {
            ...
            if ($request->hasFile('image')) {
                ...
                $imagePath = $request->file('image')->store('woods', 'public');
                $validatedData['imagePath'] = $imagePath; // Assign the path to validated data
            }

            // Store the texture in the database
            $texture = Wood::create($validatedData);

            // Return JSON response for AJAX
            return response()->json([
                'message' => 'Wood added successfully!',
    ...
    public function update(Request $request, $id)
    {
        try {
            ...
            if ($request->hasFile('image')) {
                ...
                if ($wood->imagePath) {
                    Storage::disk('public')->delete($wood->imagePath);
                }

                // Store the new image and update the image path in validated data
                $validatedData['imagePath'] = $request->file('image')->store('woods', 'public');
            } else {
                // Keep the existing image path if no new image is uploaded
                $validatedData['imagePath'] = $wood->imagePath;
            }

            // Update the texture in the database
            $wood->update($validatedData);
    ...
    public function destroy($id)
    {
        try {
            ...
            $wood->delete(); // This will set the 'deleted_at' column
            
            return response()->json(['message' => 'Wood deleted successfully!'], 200);
        } catch (\Exception $e) {
